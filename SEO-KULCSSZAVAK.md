# SEO Kulcsszó stratégia — Elektromontag

## Fő kulcsszavak (magas keresési volumen)

| Kulcsszó | Prioritás | Megjegyzés |
|---|---|---|
| autóvillamosság Budapest | 1 | Legfontosabb fő kulcsszó |
| autóvillamossági szerviz Budapest | 1 | Közvetlen szolgáltatás keresés |
| autóvillamosság 20. kerület | 1 | Helyi keresés |
| autóvillamosság Pesterzsébet | 1 | Helyi keresés |
| autóvillamossági szerelő Budapest | 2 | Személyre irányuló keresés |

## Szolgáltatás-specifikus kulcsszavak

### Önindító & Generátor (magas keresettség)
| Kulcsszó | Prioritás |
|---|---|
| önindító javítás Budapest | 1 |
| generátor javítás Budapest | 1 |
| önindító csere Budapest | 2 |
| generátor csere Budapest | 2 |
| autó nem indul Budapest | 2 |

### Klíma (szezonális — tavasz/nyár csúcs)
| Kulcsszó | Prioritás |
|---|---|
| autóklíma feltöltés Budapest | 1 |
| klíma feltöltés Budapest | 1 |
| autóklíma javítás Budapest | 2 |
| autó klíma nem hűt | 2 |
| régi típusú klímagáz feltöltés | 2 |
| R134a klímagáz feltöltés Budapest | 3 |
| R1234yf klímagáz feltöltés Budapest | 3 |
| klíma szivárgás javítás autó | 3 |

### Diagnosztika (magas keresettség)
| Kulcsszó | Prioritás |
|---|---|
| hibakód olvasás Budapest | 1 |
| autódiagnosztika Budapest | 1 |
| hibakód kiolvasás árak | 2 |
| motortér lámpa ég Budapest | 2 |
| OBD diagnosztika Budapest | 3 |

### Akkumulátor (egész évben keresett, télen csúcs)
| Kulcsszó | Prioritás |
|---|---|
| akkumulátor csere Budapest | 1 |
| akkumulátor bevizsgálás Budapest | 2 |
| autó akkumulátor vásárlás Budapest | 2 |
| akku teszt Budapest | 3 |

### Biztonsági rendszerek (légzsák, ABS, duda)
| Kulcsszó | Prioritás |
|---|---|
| légzsák javítás Budapest | 2 |
| ABS javítás Budapest | 2 |
| autó biztonsági rendszer javítás Budapest | 2 |
| légzsák lámpa ég Budapest | 2 |
| ABS lámpa ég javítás Budapest | 2 |
| duda javítás autó Budapest | 3 |
| duda csere autó Budapest | 3 |

### Világítás (nappali menetfény, ködlámpa)
| Kulcsszó | Prioritás |
|---|---|
| nappali menetfény beszerelés Budapest | 2 |
| DRL beszerelés Budapest | 3 |
| ködlámpa beszerelés Budapest | 2 |
| ködlámpa javítás autó Budapest | 3 |

### Riasztó és lopásgátló (egész évben keresett)
| Kulcsszó | Prioritás |
|---|---|
| riasztó beszerelés autó Budapest | 2 |
| lopásgátló beszerelés Budapest | 2 |
| autóriasztó szerelés Budapest | 2 |
| indításgátló beszerelés Budapest | 2 |
| autó riasztó kiszerelés Budapest | 3 |
| rejtett kapcsoló beszerelés autó | 3 |

### Egyéb szolgáltatások
| Kulcsszó | Prioritás |
|---|---|
| ablakemelő javítás Budapest | 2 |
| központi zár javítás Budapest | 2 |
| tolatókamera beszerelés Budapest | 2 |
| tolatóradar beszerelés Budapest | 2 |
| vonóhorog bekötés Budapest | 2 |
| autóhifi beszerelés Budapest | 2 |
| menetrögzítő kamera beszerelés | 3 |
| fűtőmotor javítás autó Budapest | 3 |
| ablaktörlő motor javítás | 3 |
| ablakmosó javítás autó Budapest | 3 |
| műszaki vizsga felkészítés Budapest | 2 |

## Long-tail kulcsszavak (alacsonyabb verseny, könnyebb rangsorolás)

- "autóvillamossági szerviz Pesterzsébet"
- "autóvillamosság Budapest XX. kerület"
- "önindító javítás azonnal Budapest"
- "generátor javítás olcsón Budapest"
- "hibakód olvasás Pesterzsébet"
- "klíma feltöltés 20. kerület"
- "akkumulátor bevizsgálás helyszínen Budapest"
- "autó elektromos hiba javítás Budapest"
- "izzó csere szerviz Budapest"
- "autó biztosíték csere Budapest"
- "légzsák lámpa világít javítás Pesterzsébet"
- "ABS lámpa ég javítás 20. kerület"
- "nappali menetfény beszerelés Pesterzsébet"
- "ködlámpa csere Budapest XX. kerület"
- "autó duda nem működik javítás Budapest"
- "régi típusú klímagáz feltöltés Budapest"
- "klímafeltöltés régi és új gázzal Budapest"
- "veszélyes hulladék akkumulátor átvétel Budapest"
- "riasztó beszerelés autó Pesterzsébet"
- "lopásgátló szerelés Budapest XX. kerület"
- "indításgátló beszerelés autóvillamosság Budapest"

## Meta leírás javaslat (max 160 karakter)

**Főoldal:**
"Autóvillamossági szerviz Budapest XX. kerületében. Önindító, generátor javítás, klímafeltöltés, hibakód olvasás. 35+ év tapasztalat. Hívj: +36 30 624 8584"

## Beépítési javaslat

Ezeket a kulcsszavakat a következő helyeken kell megjeleníteni:
1. **Meta title és description** — Layout.astro
2. **H1, H2 címek** — Hero, Services szekciók
3. **Alt tagek** — képeknél (ha lesznek)
4. **Szolgáltatás leírások** — Services.astro
5. **JSON-LD strukturált adat** — Layout.astro (már benne van)
6. **URL struktúra** — ha később aloldalak lesznek
